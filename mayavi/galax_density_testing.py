...
x, y, z = np.ogrid[min(xs):max(xs):DIVS*1j, min(ys):max(ys):DIVS*1j, min(zs):max(zs):DIVS*1j]
...
scalars = densityfunc(x,y,z)

obj=mlab.contour3d(scalars,contours=2,transparent=True)
mlab.show()
#fig=plt.figure()
